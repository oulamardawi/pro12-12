...
const handleLikeRed=(item)=>{ 
        ...
        fetch("http://10.0.2.2:3000/likedComm")
        .then(res=>res.json())
        .then(res=>{
          ...
          for (let i = 0 ;i<res.length;i++){
            if (res[i].student==commId){
              ...
              fetch("http://10.0.2.2:3000/update-likedCommPosts",{
            method :"post",
            headers:{
        ...
        .then(res=>{
          ...
          for (let i = 0 ;i<res.length;i++){
            if (res[i].studentId==commId){
              ...
              fetch("http://10.0.2.2:3000/update-studentComms",{
            method :"post",
            headers:{
                'Content-Type':'application/json'
            },
    ...
    function renderPosts() {
      ...
      .then(results=>{
        for (let i=0;i<results.length;i++){
          if (results[i].student==commId){
            var s = results[i].posts;
            var ss = s.split('+');
            for (let j =0;j<ss.length;j++){
